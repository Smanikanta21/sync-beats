    ...
    <>
      {items.map((item) => (
        <div
          ...
          onClick={() => item.onClick && item.onClick()}  // ← Add this
        >
          <AnimatePresence>
          ...
          <Image
            ...
            src={item.image}
            alt={item.name}
            className="relative !m-0 h-14 w-14 rounded-full border-white/60 border object-cover object-top !p-0 transition duration-500 group-hover:z-30 group-hover:scale-105"
          />
        </div>
      ))}
    </>
